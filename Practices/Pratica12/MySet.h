...
private:
	...
	//Função auxiliar para etapa 1
	void Destroy(Node<T> *);
	Node<T> *copia(Node<T> *, Node<T> *) const;
...
template <class T>
Node<T> * MySet<T>::copia(Node<T> *otherRoot, Node<T> *prev) const{
	if(otherRoot == NULL) 
		return otherRoot;

	Node<T> *tree = new Node<T>(otherRoot->elem);

	tree->parent = prev;
	tree->left = copia(otherRoot->left, tree);
	tree->right = copia(otherRoot->right, tree);
...
MySet<T> & MySet<T>::operator=(const MySet<T>&other){
	if(this == &other) 		//verificando autoatribuição
		return *this;
