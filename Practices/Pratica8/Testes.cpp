...
//Criando iterators auxiliares para auxiliar nas mudanças
int elementos =0;
typename MyList2<T>::iterator first = lista.begin();
typename MyList2<T>::iterator last = lista.begin();  

if(first==lista.end())return;
  while(first != lista.end()){
    elementos ++;
    last = first;
    ++first;
  }
  ...
  for(int i=0; i<=elementos/2; i++){
    if(elementos%2==0 && i==elementos/2){
      first++;
      last--;
    }
    else{
